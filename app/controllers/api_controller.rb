...
def addpatientmsg      #new text message add api
  ...
  def addnotes      #new text message add api
    if params[:id].empty?
        notes = ActiveSupport::HashWithIndifferentAccess.new
        notes[:success] = 0
        notes[:msg] = 'empty patient id'
        notes[:data] = ''
        respond_with(notes)
    elsif params[:physician].empty?
        notes = ActiveSupport::HashWithIndifferentAccess.new
        notes[:success] = 0
        notes[:msg] = 'empty physician data'
        notes[:data] = ''
        respond_with(notes)
    elsif params[:text_msg].empty?
        notes = ActiveSupport::HashWithIndifferentAccess.new
        notes[:success] = 0
        notes[:msg] = 'empty message data'
        notes[:data] = ''
        respond_with(notes)
    elsif
        @patient = Patient.where(:id => params[:id])
        @notes = params[:text_msg]
          if @patient.empty?
            notes = ActiveSupport::HashWithIndifferentAccess.new
            notes[:success] = 0
            notes[:msg] = 'patient not available'
            notes[:data] = ''
            respond_with(notes)
          else
            ...
            @note.physician = params[:physician]
            @note.text_msg = @notes
            if @note.save
               notes = ActiveSupport::HashWithIndifferentAccess.new
               notes[:success] = 1
               notes[:msg] = 'message save succesfully'
               notes[:data] = @note
               respond_with(notes)
            else
               note = ActiveSupport::HashWithIndifferentAccess.new
               note[:success] = 0
               note[:msg] = 'message not save'
               note[:data] = ''
               respond_with(note)
            end 
  ...
  def noteslist
      @notelist = Note.where(:patient_id => params[:id])  #id of the patientid id
      if @notelist.empty?
        notelist = ActiveSupport::HashWithIndifferentAccess.new
        notelist[:success] = 0
        notelist[:msg] = 'data not available'
        notelist[:data] = ''
        respond_with(notelist)     
      else
        notelist = ActiveSupport::HashWithIndifferentAccess.new
        notelist[:success] = 1
        notelist[:msg] = 'Succesfully get data'
  ...
  def notesedit
    @notesedit = Note.where(:note_msg_id => params[:note_msg_id])  #id of the pataintmsg id
    if @notesedit.empty?
        notesedit = ActiveSupport::HashWithIndifferentAccess.new
        notesedit[:success] = 0
        notesedit[:msg] = 'data not available'
        notesedit[:data] = ''
        respond_with(notesedit)     
    else
      Note.where(:note_msg_id => params[:note_msg_id]).update_all('text_msg' => params[:text_msg])
      @notesedit = Note.where(:note_msg_id => params[:note_msg_id])  #id of the pataintmsg id
      notesedit = ActiveSupport::HashWithIndifferentAccess.new
      notesedit[:success] = 1
      notesedit[:msg] = 'data updated'
      notesedit[:data] = ''
      respond_with(notesedit)   
    end  
